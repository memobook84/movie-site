export default function Loading() {
  return (
    <main className="min-h-screen pt-14 md:pt-24 pb-28 px-6 md:px-16">
      <div className="flex flex-col items-start gap-6 sm:flex-row">
        <div className="w-32 sm:w-40 aspect-[2/3] animate-pulse rounded-[4px] bg-gray-200" />
      ...
      <div className="mt-12">
        ...
        <div className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="aspect-[2/3] animate-pulse rounded-[4px] bg-gray-200" />
          ))}
        </div>
      </div>
    </main>
